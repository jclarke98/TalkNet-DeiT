...
def generate_results(track_direc, split, save_res):
    # track_direc is location of the tracking results in the format of .txt files per video clip

    """
    Assumes ASD output is in format vid_id:trackid:pid.json
    Assumes track_direc is in format vid_id.txt
    Assumes split.list is in relative base direc i.e {here}/utils/{this_file}

    Will save in format {vid_num}.txt
    """
